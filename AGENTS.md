...
| **Finish** (v5.9; body v2) | Final-pass preset for a keeper she already loves (typically promoted back in via Use-as-source). One pass of painterly resolution "the way a fine artist resolves a canvas". Body v2 (same day, Jeff: "push the contrast a bit more and the dimensionality") COMMANDS the push — "noticeably deeper value contrast", darks richer while lights hold, forms turning decisively — with the pin "push value and form, never hue". Fenced like Etching (painting-not-digital-rendering + no-redraw + no-new-elements DO/DON'Ts) because the failure mode is over-rendering into 3D/airbrush slickness. Works on every engine tier (preset bodies pass through per §17; engine accents differ — Pro balanced, Max thickest, Seedream lightest). Validated July 27 on a live keeper, v1 and v2 draws. | Every line, character, shape, the composition, the hue palette, the level of abstraction. Values/contrast and form-modeling are what move. |

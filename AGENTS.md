...
| **Finish** (v5.9) | Final-pass preset for a keeper she already loves (typically promoted back in via Use-as-source). One pass of painterly resolution "the way a fine artist resolves a canvas": subtle modeling and value depth in existing shadows, forms turning a little more where they already turn, smallest fabric folds where cloth already bends — blue-chip, museum-quality surface confidence. Fenced like Etching (explicit painting-not-digital-rendering + no-redraw DO/DON'Ts) because the failure mode is over-rendering into 3D/airbrush slickness. Works on every engine tier (preset bodies pass through per §17). Sanity-validated July 27 on a live keeper: modeling deepened, zero drift, nothing added. | Everything: every line, character, shape, color, the composition, the palette, the level of abstraction. Only depth/modeling within what already exists changes. |

...
| **Finish** (v5.9; body v3) | Final-pass preset for a keeper she already loves (typically promoted back in via Use-as-source). One pass of painterly resolution focused on the LIGHT: make it "rich, luminous, and high quality" but **cast exactly the way it already is** — same direction, same sources, same shadow placement, with an explicit "do not darken the painting overall" legibility guard. Body lineage, all validated on the same live keeper July 27: v1 (subtle modeling — Jeff: too timid), v2 (global contrast push — REJECTED, "too hard to read"; do not resurrect global-contrast framing), v3 (rich light — REJECTED, model reached for saturation: "vibrant but no beautiful light and shadow"), v4 (half-tone light program — right light, but the half-tones taught the model to BLEND and her chunky strokes smoothed into soft rendering), v5 (current: v4's light program + the stroke anchor — every transition built "stroke by stroke, never by blending or softening"; the anti-vibrance ban and the same-casting pin both stay). v6 (current, same day — Jeff: v5 still read as "post process on a 2d plane… imagine the light cast in a 3d world"): commands SCENE reasoning — read the painting as a real 3D scene, find her implied source, then paint physical illumination (struck surfaces, form shadow, directional cast shadows with weight, occlusion darks, reflected warmth inside deep shadow) — with an explicit "never add light as a flat glow / vignette / layered brightness" ban. Validated: bottles cast source-consistent shadows, figure casts onto table, strokes intact. v6's KNOWN TRADEOFF (faces re-modeled by form shadow) was hit immediately on the live keeper — v7 (current, same day) adds the MODEL-INTEGRITY clause via Jeff's 3D-animation framing: her drawing is the finished model; light wraps it but never re-models it; "a face in shadow is still exactly her face — same features, same lines, fully readable, only darker." Validated on the keeper: face + hand geometry intact under full scene light (raking diagonals, source-consistent cast shadows). The CG concept is used WITHOUT CG vocabulary that would invite a render look (no "3d animation" wording in the body — it says "a finished model that the light merely falls upon"). v8 (current, same day — Jeff: v7 cast light onto the keeper's shadowed feet) adds the ONE-DIRECTIONAL SHADOW-CONSERVATION law, derived from the asymmetry in his feedback (invented bottle cast-shadows = praised; light invading her painted shadow = wrong): her painted shadows are authoritative — deepen/enrich but "never let light into it"; new cast shadows may fall where the scene demands; "new light may never appear where she painted dark." Validated: feet/doorway darks preserved, v7's invented raking wedge gone, cast shadows + face integrity retained. v9 (current, same day — Jeff: "v8 feels a bit more just saturated"; measured mean-sat keeper 38 → v8 55/58 across two draws): v8's shadow clause had used CHROMA VERBS ("enrich", "warm its darkness") — v9 swaps them for value verbs ("deepen it and give it quiet depth") and adds the match-her-saturation pin ("match her hues and her saturation exactly… all richness come from value, never from color"). Validated: v9 draw measures 36.9 (at-keeper), shadow law + casts + face + strokes all retained. Saturation measurement is the cheap objective check for any future Finish body edit — compute mean HSV-sat at 128px and compare against the keeper before shipping. v10 (current, same day — Jeff on v9: "changes the face too much"): v7's model-integrity clause still PERMITTED repainting faces ("only darker") and every repaint drifted — v10 removes faces from the operation entirely: "reproduce every face exactly as she painted it — same strokes, same values, same colors, untouched… the scene's light lives around the faces, never on them." Physically inconsistent (an unlit face amid scene light) and deliberately so — her faces are small, flat, iconic; identity outranks illumination physics. Validated across two draws: faces near-indistinguishable from the keeper's (zoom crops), sat 48.1/41.1 (mean ≈ v7's 44 — draw variance, not chroma drift). Six laws now compose: scene reasoning (v6), model integrity (v7), shadow conservation (v8), stroke anchor (v5), value-not-chroma (v9), face exclusion (v10). The lesson across six bodies: each new permission needs a paired anchor to her mark-making, or the model spends her style to satisfy the permission — and light language must be SCENE language, or the model composites in image space. Fenced like Etching (painting-not-digital-rendering + no-redraw + no-new-elements DO/DON'Ts) because the failure mode is over-rendering into 3D/airbrush slickness. Works on every engine tier (§17; engine accents — Pro balanced, Max thickest, Seedream lightest). | Every line, character, shape, the composition, the palette, the shadow placement, overall legibility, the level of abstraction. Only the RICHNESS of the existing light (and gentle deepening of existing shadows) moves. |

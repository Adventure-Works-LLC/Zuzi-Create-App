...
/**
 * Hard-delete a source: collects all R2 keys belonging to it (input image +
 * every tile's output + thumb across every iteration) BEFORE the DB delete,
 * then nukes the source row. The FK cascade on iterations and tiles drops
 * those rows automatically.
 *
 * Returns the collected R2 keys so the caller can run the R2 cleanup pass
 * after the transaction commits. Order: DB-first, R2-best-effort-after.
 * If the DB transaction commits but R2 cleanup partially fails, we have
 * orphaned R2 objects (acceptable — a future sweep job picks them up; user
 * sees the source gone). If we did R2-first and DB failed, we'd have broken
 * DB rows pointing at missing R2 keys (visible as broken images for any
 * other browser tab still open against the about-to-be-deleted source).
 *
 * Returns null if the source didn't exist.
 */
export interface HardDeleteSourceResult {
  sourceId: string;
  r2Keys: string[];
...
export function hardDeleteSource(
  sourceId: string,
): HardDeleteSourceResult | null {
  return db().transaction((tx) => {
    const src = tx.select().from(sources).where(eq(sources.id, sourceId)).get();
    if (!src) return null;

    // Collect R2 keys before the cascade nukes the rows. We pull every tile
    // (active OR soft-deleted) since we're hard-deleting the whole subtree —
    // soft-deleted tiles still occupy R2 storage and need cleanup too.
    const iterRows = tx
    ...
    const tileRows =
      iterIds.length === 0
        ...
        : tx
            ...
            .from(tiles)
            .where(inArray(tiles.iteration_id, iterIds))
            .all();

    const r2Keys: string[] = [src.input_image_key];
    for (const t of tileRows) {
      if (t.output_image_key) r2Keys.push(t.output_image_key);
      if (t.thumb_image_key) r2Keys.push(t.thumb_image_key);
    }

    // FK cascade on iterations + tiles drops everything when the source row
    // goes. Single statement, atomic.
    tx.delete(sources).where(eq(sources.id, sourceId)).run();

    return {
      sourceId,
      r2Keys,
      iterationCount: iterIds.length,

...
const FINISH_PROMPT_BODY = `this painting is finished in its composition, style, and color — give it one final pass of painterly resolution focused on the light. make the light rich, luminous, and high quality — but cast exactly the way it already is in this painting: same direction, same sources, same shadow placement. do not relight it, do not add new light sources, and do not darken the painting overall — it must stay as easy to read as it is now. let the lit passages glow with richer, more confident paint, and let the existing shadows sit quietly deeper only where they already are, so the forms turn gently where they already turn. this is painting, not digital rendering: keep the exact same style, palette, shapes, composition, and level of abstraction. every line, every character, every shape stays exactly where it is. do not make it realistic, do not make it 3d, airbrushed, or digitally smooth. do not add new elements or details that are not already there. do not redraw anything. the result should read as a blue-chip fine art painting — confident, resolved, museum-quality — and unmistakably the same painting by the same painter.`;

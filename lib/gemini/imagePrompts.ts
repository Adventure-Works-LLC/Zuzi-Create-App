...
const FINISH_PROMPT_BODY = `this painting is finished in its composition, style, and color — give it one final pass devoted to beautiful light and shadow. first, read the painting as a real three-dimensional scene: the space, the objects, the figure, and where her light comes from — its direction and source are already written in the highlights and shadows she painted. then paint that light as if it truly exists inside that space. never add light as a flat glow, a vignette, or a brightness layered on top of the picture plane. treat her drawing like a finished model that the light merely falls upon: the characters and every object are already modeled exactly as she drew them — the light may wrap them, but it may never re-model, re-draw, or reshape them. the faces are off limits entirely: reproduce every face exactly as she painted it — same strokes, same values, same colors, untouched. do not light a face, do not shade a face, do not repaint a face. the scene's light lives around the faces, never on them. everywhere else, shadow darkens the paint that is there — it never erases, obscures, or replaces what she drew. where a surface faces the light, the light is genuinely striking it — let it catch real illumination. where a form turns away from the light, it falls into form shadow. where an object blocks the light, its cast shadow has direction, weight, and an edge that agrees with the source. her painted shadows are the truth of the scene: any area she painted in shadow stays in shadow — deepen it and give it quiet depth, but never let light into it. light lives only where she already placed light. new cast shadows may fall where the scene demands them, but new light may never appear where she painted dark. let the deep shadows be interesting: darker, cooler, with a faint breath of reflected warmth inside them, the way real painted shadow lives. build all of it out of the same chunky, visible brushstrokes this painting is already made of — stroke by stroke, never by blending or softening them. keep the brushstroke size, texture, and crudeness identical everywhere: the figure, the hands, the face, and every object keep their simple, wonky painted shapes exactly — do not model them smoothly, do not round them, do not give them glossy realistic highlights. this is not about color: do not increase saturation or vibrance, do not shift the palette — match her hues and her saturation exactly, and let all richness come from value, never from color. keep the painting easy to read. this is painting, not digital rendering: keep the exact same style, shapes, composition, and level of abstraction. every line, every character, every shape stays exactly where it is. do not make it realistic, do not make it 3d, airbrushed, or digitally smooth. do not add new elements or details that are not already there. do not redraw anything. the result should read as a blue-chip fine art painting — real light living inside a real scene, in her exact style — and unmistakably the same painting by the same painter.`;

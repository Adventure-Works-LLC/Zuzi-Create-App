...
const FINISH_PROMPT_BODY = `this painting is finished in its composition, style, and color — give it one final pass devoted to beautiful light and shadow. the light is already cast: same direction, same sources, same shadow placement — never relight it. what changes is the quality of that light. let the lit passages feel genuinely lit — a soft, luminous gradient across them instead of a flat fill. let the shadows feel like real painted shadow: gently deeper and slightly cooler where they already are, transparent and alive, never black or muddy. paint half-tone transitions where light turns into shadow — that transition is where the beauty lives — but build every transition out of the same chunky, visible brushstrokes this painting is already made of. the gradient happens across her strokes, stroke by stroke, never by blending or softening them. keep the brushstroke size, texture, and crudeness identical everywhere: the figure, the hands, the face, and every object keep their simple, wonky painted shapes exactly — do not model them smoothly, do not round them, do not give objects glossy realistic highlights. tuck the smallest dark accents into contact points that are already dark. this is not about color: do not increase saturation or vibrance, do not shift the palette — the beauty must come from value and transition, not chroma. keep the painting easy to read. this is painting, not digital rendering: keep the exact same style, shapes, composition, and level of abstraction. every line, every character, every shape stays exactly where it is. do not make it realistic, do not make it 3d, airbrushed, or digitally smooth. do not add new elements or details that are not already there. do not redraw anything. the result should read as a blue-chip fine art painting — museum light, confident and resolved — and unmistakably the same painting by the same painter.`;

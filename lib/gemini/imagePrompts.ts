 ...
 *   - `presets: [<one or more>]` → builder constructs a "vary X, preserve Y"
 *     prompt. The vary list is the labels of the checked presets; the
 *     preserve list is the master preserve list MINUS items removed by each
 *     checked preset (e.g. `lighting` removes both "lighting" and "value
 *     structure" because lighting drives values).
 *
 * Preset order in the rendered prompt is FIXED (color → composition → lighting
...
/** What each preset commands the model to vary, when checked. */
const PRESET_LABEL: Record<Preset, string> = {
  color: "the colors and palette",
  composition: "the composition, framing, and how elements are arranged within the frame",
 ...
 * because changing lighting necessarily changes values). */

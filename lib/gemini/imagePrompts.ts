...
const FINISH_PROMPT_BODY = `this painting is finished in its composition, style, and color — give it one final pass of painterly resolution, the way a fine artist resolves a canvas. push it: noticeably deeper value contrast and a stronger sense of form and dimensionality, built entirely with paint in the same style. let the darks go richer and deeper while the lights hold their ground. model the forms so they turn decisively where they already turn, and deepen the folds where clothing already bends. this is painting, not digital rendering: keep the exact same style, palette, shapes, composition, and level of abstraction — push value and form, never hue. every line, every character, every shape stays exactly where it is. do not make it realistic, do not make it 3d, airbrushed, or digitally smooth. do not add new elements or details that are not already there. do not redraw anything. the result should read as a blue-chip fine art painting — confident, resolved, museum-quality — and unmistakably the same painting by the same painter.`;

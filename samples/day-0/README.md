...
Color v3; v4 lives on at `untitled_artwork_3-background-v4/` and v3 at
`untitled_artwork_3-background-v3/` as historical references); the Color
prompt is now at v3 (v1 templated → v2 cel-animation lock did wholesale
palette replacement and drifted skin/mood → v3 anchors on HER existing
palette as the base, exempts skin tones from any shift, and explicitly
preserves her peaceful/warm mood register; v2 lives on at
`untitled_artwork_3-color-v2/` as a historical reference). Per-prompt
lineage and the load-bearing-redundancy + anti-language + judgment-
imitation + construct-not-just-surface + mood-override + skin-identity +
read-and-develop lessons are documented in the comment blocks above each
